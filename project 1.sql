...
use login1;
create table loan_status4(Loan_ID varchar(80),	Customer_id varchar(80) ,	 Loan_Amount varchar(70),	Loan_Amount_Term int,	Cibil_Score int ,primary key(loan_id));
create table remark_update4(Loan_id varchar(80),	Loan_amount varchar(70),	Cibil_Score int,	Cibil_Score_status varchar(100),primary key (loan_id));
  drop table loan_status4;
  drop table remark_update4;
 -- trigger
 
 delimiter //
 create trigger remark_sts5 after insert on loan_status4 for each row
 begin
 if new.cibil_score>900 then insert into remark_update4(Loan_id,Loan_amount,Cibil_Score,Cibil_Score_status) values
 (new.loan_id,new.loan_amount,new.cibil_score,'high_cibil_score');
 elseif new.cibil_score>750 then insert into remark_update4(Loan_id,Loan_amount,Cibil_Score,Cibil_Score_status)values
 (new.loan_id,new.loan_amount,new.cibil_score,'no_penalty');
 elseif new.cibil_score>250 then insert into remark_update4(Loan_id,Loan_amount,Cibil_Score,Cibil_Score_status)values
 (new.loan_id,new.loan_amount,new.cibil_score,'penalty_customers');
 elseif new.cibil_score<=0 then insert into remark_update4(Loan_id,Loan_amount,Cibil_Score,Cibil_Score_status)values
 (new.loan_id,new.loan_amount,new.cibil_score,'reject_customers');
 end if ;
 end //
 delimiter ;


drop trigger remark_sts5;
 ...
 delimiter //
 create trigger load_amount_check before insert on loan_status4 for each row
 begin
 ...
 select * from loan_status4;
  select * from remark_update4;
 
 show triggers;
 
 select * from load_amount_check4;
 
 insert into loan_status4 values
...
create table region_table select country_state.customer_id,region_cus.region,region_cus.region_id 
from country_state right join region_cus on country_state.region_id=region_cus.region_id;

create table cibil_score_sts select loan_status4.loan_id,loan_status4.customer_id,remark_update4.loan_amount,loan_status4.loan_amount_term,loan_status4.cibil_score,
remark_update4.cibil_score_status from loan_status4 inner join remark_update4 on loan_status4.loan_id=remark_update4.loan_id;


 
drop table cibil_score_sts;

select*from cibil_score_sts;
...
create table table4 select c.customer_id,i.applicantincome,i.property_area,c.cibil_score,ic.gender,c.loan_amount,ic.customer_name,cs.state,r.region,r.region_id
from cibil_score_sts as c inner join income_sts as i on c.loan_id=i.loan_id inner join country_state as cs on
c.customer_id=cs.customer_id inner join info_customer as ic on c.loan_id=ic.loan_id inner join region_table as r on r.customer_id=c.customer_id;
...
select * from cibil_score_sts;
select * from loan_status4;
select * from region_cus;
select * from remark_update4;
-- task
create table intrest_calculation select *,
case when applicantincome >15000 then "A_grade_customer"
when applicantincome>9000 then "B_grade_customer"
when applicantincome>5000 then "middle_class_customer"
else "low_class_Customer" end as customer_criteria,
...
when applicantincome<5000 and property_area="semiurban" then "2.5" else"7"
end as monthly_intrest_calculation
from table4;

select * from intrest_calculation;

delete from intrest_calculation where loan_amount='Loan still processing';

create table amount_calculation select *, round((loan_amount*monthly_intrest_calculation/100),0) as monthly_amount,
round((loan_amount*monthly_intrest_calculation/100)*12,0) as Annual_amount from intrest_calculation;
drop table amount_calculation;
select * from amount_calculation;

delimiter --
create procedure Bank_loan()
begin
select c.customer_id,i.applicantincome,i.property_area,c.cibil_score,ic.gender,c.loan_amount,ic.customer_name,cs.state,r.region,r.region_id
from cibil_score_sts as c inner join income_sts as i on c.loan_id=i.loan_id inner join country_state as cs on
c.customer_id=cs.customer_id inner join info_customer as ic on c.loan_id=ic.loan_id inner join region_table as r on c.customer_id=r.customer_id;

select * from amount_calculation;

select state,count(customer_name) as total from country_state group by state;

select state, sum(loan_amount) as sum from intrest_calculation group by state;
 
select state,sum(applicantincome) as applicant_sum from income_sts inner join country_state on income_sts.customer_id=country_state.customer_id group by state;

select customer_name ,right (customer_name,4)as names from country_state where customer_name like "% ____";
...
update info_customer set gender='female' where customer_Id="IP43006";
update info_customer set gender="female" where customer_id="IP43016"; 
update info_customer set gender="male" where customer_id="IP43018"; 
update info_customer set gender="male" where customer_id="IP43038"; 
update info_customer set gender="female" where customer_id="IP43508"; 
update info_customer set gender="female" where customer_id="IP43577"; 
update info_customer set gender="female" where customer_id="IP43589"; 
update info_customer set gender="female" where customer_id="IP43593"; 

update info_customer set age=45 where customer_id="IP43007";
update info_customer set age=32 where customer_id="IP43009";  
select * from info_customer;

create table calculation select monthly_intrest_calculation ,round((applicantincome*monthly_intrest_calculation/100),1) as monthly_intrest_amount,round((applicantincome*monthly_intrest_calculation/100)*12,1) as annual_intrest_amount
from intrest_calculation;
